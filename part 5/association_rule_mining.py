...
# SPECIFICATION: A simple program that uses a csv of transactions to do association rule mining
...
print("what", df.iterrows)
encoded_vals = []
for index, row in df.iterrows():
    labels = dict.fromkeys(itemset, 0)
    for item in row:
        if item in labels:
            labels[item] += 1
...
for i in range(len(rules)):
    a = str(rules['antecedents'][i])
    a = a.replace('frozenset({', '')
    a = a.replace('})', '')
    antecedents.append(a)
    b = str(rules['consequents'][i])
...
for i in range(len(rules)):
    ...
    print("look for this", consequents[i])
    print("Support", rules['support'][i])
    print("Confidence", rules['confidence'][i])
    rule_confidence = rules['confidence'][i]
    suportCount = consequents.count(consequents[i])
    print("supp", suportCount)
    prior = suportCount / len(encoded_vals)
    print("Gain in Confidence: " + str((rule_confidence-prior)/prior))
